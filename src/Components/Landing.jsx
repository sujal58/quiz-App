    ...
    <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 2xl:grid-cols-2 h-screen ${themeMode} ${textMode}`}>
        ...
        <div className='flex justify-center flex-col items-center font-serif text-xl'>
          {optionToChoose.map((optionVal, index)=>{
            return(<div className={`flex items-center justify-center h-12 w-80 rounded-xl mb-4 cursor-pointer gap-4 text-slate-800 ${themeMode} ${textMode} ${borderColor} box-border border-solid border-2 ring-2 ring-slate-800`} onClick={() => handleClick(optionVal)} key={index}>
